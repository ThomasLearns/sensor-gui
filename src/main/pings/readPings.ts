import { ReadlineParser, SlipDecoder, SlipEncoder } from 'serialport'
...
export function readPings(device: Device<'port'>) {
  ...
  // )
  const parser: SlipDecoder = device.port.pipe(new SlipDecoder())
  device.parser = parser

  const encoder = new SlipEncoder();

  const dataRequestPacket = Buffer.alloc(1, 0x01)

  encoder.on('data', (data: unknown) => {
    device.port.write(data);
  })
  encoder.push(dataRequestPacket);

  parser.on('data', (packet: unknown) => {
    console.log(packet)
    encoder.push(dataRequestPacket);
    return
